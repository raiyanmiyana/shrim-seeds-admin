    ...
    fieldState
      ...
      <div className="position-relative">
        ...
        <span className="d-flex position-absolute top-50 end-0 translate-middle-y p-0 pe-2 me-1" onClick={() => setShowPassword(!showPassword)}>
          {!fieldState.error && (showPassword ? <IconifyIcon icon="ri:eye-off-line" height={18} width={18} className="cursor-pointer" /> : <IconifyIcon icon="ri:eye-line" height={18} width={18} className="cursor-pointer" />)}
        </span>
      </div>
    </FormGroup>} />;
};
