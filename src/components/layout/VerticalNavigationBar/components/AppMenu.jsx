...
const MenuItemWithChildren = ({
  ...
  return <li className={className}>
      <div onClick={toggleMenuItem} data-bs-toggle="collapse" aria-expanded={open} role="button" className={clsx(linkClassName)}>
        ...
        {!item.badge ? <span className="menu-arrow">
            <IconifyIcon icon="ri:arrow-drop-right-line" width={24} height={24} />
          </span> : <span className={`badge badge-pill text-end bg-${item.badge.variant}`}>{item.badge.text}</span>}
      </div>
      <Collapse in={open}>
        <div>
          <ul className={clsx(subMenuClassName)}>
            ...
            return <Fragment key={child.key + idx}>
                  {child.children ? <MenuItemWithChildren item={child} linkClassName={clsx('side-nav-link', getActiveClass(child))} activeMenuItems={activeMenuItems} className={clsx('side-nav-item', activeMenuItems?.includes(child.key) ? 'menuitem-active' : '')} subMenuClassName="side-nav-second-level" toggleMenu={toggleMenu} /> : <MenuItem item={child} className={clsx('side-nav-item', activeMenuItems?.includes(child.key) ? 'menuitem-active' : '')} linkClassName={clsx('side-nav-link', getActiveClass(child))} />}
                </Fragment>;
          })}
          </ul>
        </div>
  ...
  return <ul className="side-nav">
      ...
      return <Fragment key={item.key + idx}>
            {item.isTitle ? <li className="side-nav-title">{item.label}</li> : <>
                {item.children ? <MenuItemWithChildren item={item} toggleMenu={toggleMenu} className={clsx('side-nav-item', activeMenuItems?.includes(item.key) ? 'menuitem-active' : '')} linkClassName={clsx('side-nav-link', getActiveClass(item))} subMenuClassName="side-nav-second-level" activeMenuItems={activeMenuItems} /> : <MenuItem item={item} linkClassName={clsx('side-nav-link', getActiveClass(item))} className={clsx('side-nav-item', activeMenuItems?.includes(item.key) ? 'menuitem-active' : '')} />}
              </>}
          </Fragment>;
    })}
    </ul>;
